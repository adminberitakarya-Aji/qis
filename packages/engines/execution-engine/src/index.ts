...
export class ExecutionEngine {
  ...
  /**
   * Places all Buy Limit Orders on the exchange for a given strategy.
   * Uses Client Order ID for idempotency protection.
   */
  ...
  /**
   * Polls exchange for placed orders, detects fills, and places TP orders.
   */
      ...
      try {
        ...
        if (order.status === 'placed' && order.exchangeOrderId) {
          ...
          if (liveOrder.status === 'closed') {
            ...
            // Immediately place TP Sell Order
        ...
        // Check if TP Sell order is FILLED
  ...
  /**
   * Cancels all open (placed) orders on exchange when strategy is stopped.
   */
  ...
  /**
   * Executes a MARKET BUY for a single grid order when price crosses the grid level.
   * Called by the Binance WebSocket Worker via the trigger-order API endpoint.
   *
  ...
  async executeSingleMarketBuy(
    ...
    // Step 2: Place TP Sell Limit immediately after fill
    if (filledPrice !== null && filledQuantity !== null) {
      // Recalculate TP based on ACTUAL fill price (not estimated grid price)
      // TP distance is derived from the original tpPrice / gridPrice ratio
